...
![](https://cf-courses-data.s3.us.cloud-object-storage.appdomain.cloud/H0pqcU2h9yVscoKD5GR_gQ.png)  

**Estimated time needed:** 30 minutes

Welcome to the **Practicing Test Driven Development** lab. Test driven development (TDD) is an approach to software development in which you first write the test cases for the code you _wish_ you had and then write the code to make the test cases pass.
...
-   Test-driven development produces high-quality code — and IBM Bob can accelerate the cycle. Bob helps you write tests, generate implementations, and iterate faster by understanding your test specifications and suggesting code that satisfies them, keeping you in the TDD flow without slowing down. Get started with a free trial at [ibm.biz/snp-bob.](https://ibm.biz/snp-bob)
...
![Open Terminal](https://cf-courses-data.s3.us.cloud-object-storage.appdomain.cloud/IBM-CD0241EN-SkillsNetwork/images/07_terminal.png "Open Terminal")
...
In the IDE on the left of your screen, navigate to the `duwjx-tdd_bdd_PracticeCode/labs/07_practicing_tdd` folder. This folder contains all of the source code that you will use for this lab.
...
<p>This is one solution for testing the functionality of update a counter.</p><pre><code class="language-python">    def test_update_a_counter(self):
...
> Notice that the failure is **AssertionError: 405 != 200**. A 405 is `405_METHOD_NOT_ALLOWED`, indicating that Flask found a route called `/counters/<name>` but it does not allow a `PUT` method. This is because you haven't written that code yet. That帴.
...
<details>
<summary>Click here for the solution.</summary>
<p>This is one solution for implementing the functionality of update a counter.</p><pre><code class="language-python">@app.route("/counters/&lt;name&gt;", methods=["PUT"])
...
</code></pre><blockquote><p>Notice that this code does not provide any error handling. What if the counter doesn't exist? You would need a new test case and then more code to check that the counter exists before incrementing it.</p></blockquote>
</details>
...
<details>
<summary>Click here for the solution.</summary>
<p>This is one solution for testing the functionality of read a counter.</p><pre><code class="language-python">    def test_read_a_counter(self):
...
> Notice again, that the failure is **AssertionError: 405 != 200**. A 405 is `405_METHOD_NOT_ALLOWED`, indicating that Flask found a route called `/counters/<name>` but it does not allow a `GET` method. This is because you haven't written that code yet. That帴.
...
<details>
<summary>Click here for the solution.</summary>
<p>This is one solution for implementing the functionality of read a counter.</p><pre><code class="language-python">@app.route("/counters/&lt;name&gt;", methods=["GET"])
...
</code></pre><blockquote><p>Notice that the code does not provide any error handling. Again, if the counter didn't exist, you would need a new test case and then more code to check that the counter exists before returning it.</p></blockquote>
</details>
...
<details>
<summary>Click here for the solution.</summary>
<p>This is one solution for testing the functionality of delete a counter.</p><pre><code class="language-python">    def test_delete_a_counter(self):
...
> Notice again, that the failure is **AssertionError: 405 != 204**. A 405 is `405_METHOD_NOT_ALLOWED`, indicating that Flask found a route called `/counters/<name>` but it does not allow a `DELETE` method. This is because you haven't written that code yet. That帴.
...
<details>
<summary>Click here for the solution.</summary>
<p>This is one solution for implementing the functionality of delete a counter.</p><pre><code class="language-python">@app.route("/counters/&lt;name&gt;", methods=["DELETE"])
...
</code></pre><blockquote><p>Notice that it does not provide any error handling. Again, if the counter didn't exist, you would need a new test case and then more code to check that the counter exists before deleting it.</p></blockquote>
</details>
...
Hopefully, you now understand how easy it is to write test cases for code that doesn't exist yet. You use the application屵irements to define the behavior that you are testing, and then you write the code that behaves that way.
...
Supercharge your TDD workflow with AI-assisted coding — try Bob free at [ibm.biz/snp-bob.](https://ibm.biz/snp-bob)
